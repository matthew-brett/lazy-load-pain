...
- `broken/` — `_skimage2.morphology.max_tree` (module name = function name)
- `fixed/` — `_skimage2.morphology._max_tree` (private implementation module)

Both use a lazy parent package (`lazy_loader.attach_stub` + `__init__.pyi`), like
`_skimage2.morphology` after the megamove.

A compatibility shim (`skimage/morphology/max_tree.py`) imports the implementation
...
Expected: **broken** shows `_skimage2.morphology.max_tree` as a **module** after
